...
export async function main(ns) {
    ...
    while (true) {
        try {
            ...
            values.push(ns.formatNumber(hacknetTotalProduction.toPrecision(5), "$0.0a") + '/s');

            headers.push("Hacknet Profit: ");
            values.push(ns.formatNumber(hacknetTotalProfit.toPrecision(5), "$0.0a"));

            headers.push("Script Income: ");
            values.push(ns.formatNumber(ns.getTotalScriptIncome()[0].toPrecision(5), "$0.0a") + '/s');

            headers.push("Script Experience: ");
            values.push(ns.formatNumber(ns.getTotalScriptExpGain().toPrecision(5), "0.00a") + '/s');
